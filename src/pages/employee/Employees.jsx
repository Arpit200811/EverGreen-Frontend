...
export default function Employees() {
  ...
  const [form, setForm] = useState({
    name: "", email: "", dob: "", mobile: "", aadharNo: "", address: "", password: "",
  });
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingEmployee) {
        ...
        const res = await API.post("/auth/register", { ...form, role: "EMPLOYEE" });
        savedEmployee = res.data;
      }
  ...
  const resetForm = () => {
    ...
    setForm({ name: "", email: "", dob: "", mobile: "", aadharNo: "", address: "", password: "" });
  };
  ...
  const handleEdit = (emp) => {
    ...
    setForm({
      name: emp.name, email: emp.email, dob: emp.dob || "",
      mobile: emp.mobile || "", aadharNo: emp.aadharNo || "", address: emp.address || "",
    });
    setShowModal(true);
  };
                  ...
                  <tr key={emp._id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-8 py-5">
                       <div className="space-y-1">
                          <p className="text-xs font-bold text-slate-700 flex items-center gap-2"><Phone size={12} className="text-emerald-500" /> {emp.mobile}</p>
                          <p className="text-[11px] font-medium text-slate-400 flex items-center gap-2"><MapPin size={12} /> {emp.address?.slice(0, 20)}...</p>
                       </div>
                    </td>
                    <td className="px-8 py-5">
                       <div className="flex flex-col">
                          <span className="text-xs font-black text-slate-700 flex items-center gap-2"><IdCard size={12} className="text-blue-500" /> {emp.aadharNo || 'N/A'}</span>
                          <span className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tight">Aadhaar Verified</span>
                       </div>
                    </td>
                    <td className="px-8 py-5">
            ...
            <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex items-center justify-center z-50 p-4">
              <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white w-full max-w-2xl rounded-[2.5rem] overflow-hidden shadow-2xl">
                <div className="px-8 py-6 bg-emerald-600 flex justify-between items-center text-white">
                  <h3 className="text-xl font-black tracking-tight">{editingEmployee ? "Refine Member Profile" : "Register New Member"}</h3>
                  <button onClick={resetForm} className="p-2 hover:bg-white/20 rounded-full transition-colors"><X /></button>
                </div>
                ...
                <div className="p-8 max-h-[70vh] overflow-y-auto">
                  <div className="flex justify-center mb-8">
                    <div className="relative group">
                      ...
                      <label className="absolute -bottom-2 -right-2 bg-emerald-600 p-3 rounded-2xl cursor-pointer shadow-xl text-white hover:scale-110 transition-transform">
                        <Camera size={18} />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormInput label="Professional Name" icon={Users} value={form.name} onChange={(v) => setForm({ ...form, name: v })} />
                    <FormInput label="Official Email" icon={Mail} value={form.email} onChange={(v) => setForm({ ...form, email: v })} />
                    <FormInput label="Contact Number" icon={Phone} value={form.mobile} onChange={(v) => setForm({ ...form, mobile: v })} />
                    <FormInput label="Date of Birth" icon={CalendarIcon} type="date" value={form.dob} onChange={(v) => setForm({ ...form, dob: v })} />
                    <FormInput label="Aadhaar ID" icon={IdCard} value={form.aadharNo} onChange={(v) => setForm({ ...form, aadharNo: v })} />
                    <FormInput label="Current Address" icon={MapPin} value={form.address} onChange={(v) => setForm({ ...form, address: v })} />
                    {!editingEmployee && (
                      <div className="md:col-span-2">
                         <FormInput label="Security Password" type="password" value={form.password} onChange={(v) => setForm({ ...form, password: v })} />
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="p-8 bg-slate-50 border-t border-slate-100 flex gap-4">
                  <button onClick={resetForm} className="flex-1 py-4 font-bold text-slate-500 hover:text-slate-800 transition-colors">Discard</button>
                  <button onClick={handleSave} disabled={loading} className="flex-[2] bg-emerald-600 text-white py-4 rounded-2xl font-black shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all active:scale-95 disabled:opacity-50">
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-[2.5rem] p-8 w-full max-w-sm text-center shadow-2xl">
              <div className="w-20 h-20 bg-rose-50 text-rose-600 rounded-full flex items-center justify-center mx-auto mb-4 border-4 border-white shadow-sm">
                <Trash2 size={32} />
              </div>
              <h3 className="text-xl font-black text-slate-900 mb-2">Are you sure?</h3>
              <p className="text-slate-500 text-sm mb-8 font-medium">This will permanently remove the employee from all records.</p>
              <div className="flex gap-3">
                <button onClick={() => setShowDeleteConfirm(null)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200">Cancel</button>
                <button onClick={() => handleDelete(showDeleteConfirm)} className="flex-1 py-3 bg-rose-600 text-white font-bold rounded-xl hover:bg-rose-700 shadow-lg shadow-rose-200">Yes, Delete</button>
              </div>
            </motion.div>
          </div>
        )}
...
const StatCard = ({ label, value, icon: Icon, color }) => (
  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 flex justify-between items-center group hover:-translate-y-1 transition-all">
    ...
    <div className={`w-14 h-14 rounded-2xl ${color} text-white flex items-center justify-center shadow-lg shadow-slate-200 group-hover:rotate-6 transition-transform`}>
      <Icon size={24} />
    </div>
  </div>
);

const FormInput = ({ label, value, onChange, type = "text", icon: Icon, placeholder }) => (
  <div className="space-y-1.5">
    ...
    <div className="relative">
      ...
      <input
        ...
        className={`w-full bg-slate-50 border-none rounded-2xl ${Icon ? 'pl-12' : 'px-4'} py-3.5 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-emerald-500/20 transition-all`}
      />
    </div>
  </div>
);